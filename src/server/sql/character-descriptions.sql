...
UPDATE `character` SET hobby='绘画、设计、收集文具', description='结丘女子高等学校学生（二期生）。性格内向安静，不擅长在人前表达。眼神比较凶恶常被误解，但其实内心非常温柔善良。擅长绘画和设计，在Liella!中负责视觉设计工作。与夏美是好朋友，两人经常一起行动。' WHERE character_id=37;

UPDATE `character` SET hobby='科学实验、收集矿石、读书', description='结丘女子高等学校学生（二期生）。性格冷静理性，喜欢科学和逻辑思考。是团队中的"头脑派"，擅长分析和规划。虽然看起来有些冷淡，但实际上非常关心同伴。喜欢做科学实验，对自然界有浓厚兴趣。在舞台上展现出与平时冷静形象不同的热情一面。' WHERE character_id=38;
